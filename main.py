...
class OptimizedRAGSystem:
        ...
        class TimingContext:
            ...
            def __exit__(self, exc_type, exc_val, exc_tb):
                if self.start_time is not None:
                    ...
                    print(
                        f"⏱️  {self.name}: {elapsed:.2f}s"
                    )  # Keep print for backward compatibility
    ...
    def _load_documents(self) -> List[Document]:
        ...
        with self._time_operation("Document Processing"):
            ...
            for i, pdf_path in enumerate(self.pdf_paths):
                ...
                print(
                    f"📄 Processing PDF {i + 1}/{len(self.pdf_paths)}: {os.path.basename(pdf_path)}"
                )
                ...
                print(
                    f"📊 Loaded {len(document)} document(s) from {os.path.basename(pdf_path)}"
                )
            ...
            print(
                f"📏 First document length: {len(all_documents[0].page_content)} chars"
            )
    ...
    def _setup_embeddings_and_vectorstore(self):
        ...
        if vectorstore_cache.exists():
            ...
            with self._time_operation("Vector Store Loading (Cached)"):
                try:
                    ...
                    self.logger.warning(
                        "Failed to load cached vector store; rebuilding. Error: %s", exc
                    )
                    self.vectorstore = None
                    vectorstore_cache.unlink(missing_ok=True)
                else:
        ...
        with self._time_operation("Embeddings & Vector Store Setup"):
            ...
            if self.documents is None:
                raise RuntimeError(
                    "Documents not loaded. Call _load_documents() first."
                )
            ...
            try:
                ...
                self.logger.warning(
                    "Skipping vector store caching due to serialization error: %s", exc
                )
                if vectorstore_cache.exists():
                    vectorstore_cache.unlink(missing_ok=True)
            else:
    ...
    def _setup_llm_and_chain(self):
        ...
        with self._time_operation("MLX Model Loading"):
            ...
            self.logger.debug(
                f"Model config - max_tokens: {self.max_tokens}, temp: {self.temperature}"
            )
...
Answer:"""
    ...
    def query(self, question: str) -> Dict[str, Any]:
        ...
        print(f"❓ Question: {question}")  # Keep print for backward compatibility

        with self._time_operation("Query Processing"):
            response = self.chain.invoke({"input": question})

        self.logger.info(
            f"✅ Generated answer (length: {len(response['answer'])} chars)"
        )
    ...
    def interactive_mode(self):
        ...
        print("🎯 Interactive Mode - Ask questions about your document!")
