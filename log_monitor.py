...
def get_active_sessions():
    ...
    try:
        ...
        if response.status_code == 200:
            return response.json()["active_sessions"]
        return []
    except:
        return []
    ...
    try:
        while True:
            ...
            if not check_api_health():
                print(f"❌ [{datetime.now().strftime('%H:%M:%S')}] API not responding")
                time.sleep(interval)
                continue
            ...
            if new_sessions:
                print(f"✅ [{datetime.now().strftime('%H:%M:%S')}] New session(s): {', '.join(new_sessions)}")
            
            # Check for removed sessions
            removed_sessions = last_sessions - current_sessions
            if removed_sessions:
                print(f"🗑️  [{datetime.now().strftime('%H:%M:%S')}] Removed session(s): {', '.join(removed_sessions)}")
...
def main():
    ...
    parser.add_argument("--once", "-o", action="store_true",
                       help="Show current logs once and exit")
